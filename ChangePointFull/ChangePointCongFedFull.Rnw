...
The first hypothesis is based on the idea that the Fed aims to maintain its autonomy by projecting an image of elite apoliticism through the types of organizations it speaks to. The second hypothesis presents a competing scenario. It is based on the idea that the Fed tries to maintain its autonomy with a more direct lobbying-type approach.
...
TODO: ADD DESCRIPTIVE GRAPHS OF CONNECTIVITY AND TOPICS

TODO: ADD INFO ON TOPIC ANALYSIS
