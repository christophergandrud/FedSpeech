...
<<ParentGlobalOpts, include=FALSE>>=
    ...
    # Drop if outside of the hearing observation period
...
\noindent\emph{Working draft. Comments welcome.}\footnote{Thank to you Rebecca Kanter and Brent Ramsey for research assistance.}
...
We gathered data on congressional hearings--in both the Senate and the House of Representatives--that included the presence of a representative of the Federal Reserve. We had access to the transcripts of all such hearings from 5 March 1997 through 13 December 2012. \todo[inline]{Add more info on sources}
...
We use change point analysis with three indicators of Congressional scrutiny. None of the indicators directly and unequivocally capture legislative scrutiny. But they do indicate it. Hopefully by triangulating with all three we will have a better understanding of the underlying scrutiny states. The first indicator is the number of hearings that Fed representatives are asked to attend \todo{Kevin, is this phrasing correct?}. This is not a perfect measure of scrutiny as the Fed may be asked to attend hearings scrutinizing not it, but another issue or economic bureaucracy. We also look at congress members' attendance at the hearings. Higher attendance implies that there hearings have a higher salience with members. \todo{cite literature on how congress members' statements are a type of signaling.} Finally, we examine how many times laughter is recorded in the testimony transcripts. Higher laughter counts could indicate that the hearings have a more jovial atmosphere and congress members who are less critical of the Fed.

We focus on the number of days hearings were held per month.\footnote{Some hearings were held over two or more days.} In the case of member attendance and laughter counts we use monthly medians, rather than individual hearings as the data points. This helps make the series more comparable. It also allows us to minimize how much our change point results are influenced by meetings with outlier attendance or laughter.

\paragraph{Monthly Testimony Counts} 


\begin{figure}
    \caption{Congressional Scrutiny Change Point Analysis}
    \label{fig:BaseCP}
<<BaseCP, echo=FALSE, fig.height=4, out.width='0.8\\linewidth'>>=
...
PresTS <- tsMonth(MainMonth$MembPresMedianNoF)
ChangePres <- cpt.mean(PresTS, method = "BinSeg")
plot(ChangePres, ylab = "Monthly Median Attendance", xlab = "")

## Laughter Median
LaughTS <- tsMonth(MainMonth$LaughMedianNoF)
ChangeLaugh <- cpt.meanvar(LaughTS)
plot(ChangeLaugh, ylab = "Monthly Median Laughter", xlab = "")
@
{\scriptsize{Red horizontal lines signify the estimated states.}}
\end{figure}

\begin{figure}
    \caption{Change Point Analysis of Congressional Field Hearings}
    \label{fig:MoreCP}
<<MoreCP, echo=FALSE, fig.height=4, out.width='0.8\\linewidth'>>=
...
\paragraph{Monthly Median Attendance}

\paragraph{Monthly Median Laughter}
