...
\begin{figure}
    ...
    \label{fig:BaseCP}
<<ScrutinyCP, echo=FALSE, fig.width=10, fig.height=6, message=FALSE, warning=FALSE>>=
...
@
{\scriptsize{The red horizontal dashed line at September 2008 signifies the estimated change point at the 95\% significance level from an analysis including all of the variables.\\
