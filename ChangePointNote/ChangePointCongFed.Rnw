...
<<ParentGlobalOpts, include=FALSE>>=
    ...
    MainMonth <- read.csv("MainHearings.csv")
...
One potential disadvantage of Matteson and James' change point method is that the minimum cluster size must be set \emph{a priori} \citeyearpar[11]{Matteson2012}. In other words, we need to tell the algorithm the minimum number of time points that are between the change points. The minimum size is generally needs to be larger if the distance between the probability distributions is small. We had no reason to impose a particular minimum cluster size on the model so we started from a size of 6 months and moved upward until we found change points significant at least the 90\% level. 
...
We also look at congress members' attendance at the hearings. Higher attendance implies that the hearings have a higher salience with members. \todo{cite literature on how congress members' statements are a type of signaling.} Again, there may be high attendance and high salience for scrutiny not aimed directly at the Fed. Unfortunately attendance reporting was inconsistent across the committees. The HCFS had complete data on attendance. The SBHUA Committee did not consistently report attendance.

Finally, we initially examined how many times instances of laughter was recorded in the testimony transcripts. Higher laughter counts could indicate that the hearings have a more jovial atmosphere and congress members who are less critical of the Fed. However, we have excluded laughter from our final analysis because the data was being driven by specific individuals at the Fed, primarily Federal Reserve Vice Chairman Roger W. Ferguson Jr. He regularly gave testimony before late 2003 when there were a number of high laughter counts. When he became the chairman of the Committee on the Global Financial System and Financial Stability Forum. Both are based in Basel, Switzerland and Ferguson did not give any Congressional testimony from that point. Laughter counts declined substantially as soon as he stopped giving testimony. Because the laughter variable is so susceptible to individual effects, we did not include it in our change point analysis.
...
<<ScrutinyHouseFedCP, echo=FALSE, message=FALSE, warning=FALSE, out.width='0.8\\linewidth'>>=
# Scrutiny Change Point
ScrutVarsHouse <- c("SumFedHouse", "SumFedSenate")
e.divGG(data = MainMonth, Vars = ScrutVarsHouse, TimeVar = "MonthYear", 
        sig.lvl = 0.1, R = 1999, min.size = 30)
@
{\scriptsize{The red horizontal dashed line signifies the estimated change point at the 90\% significance level from an analysis including both of the variables.\\
The data is aggregated on a monthly basis, i.e. observations represent monthly hearing counts.}}
\end{figure}


\begin{figure}
    \caption{Congressional Scrutiny Change Point Analysis: US House Financial Services Committee with No Federal Reserve Testimony}
    \label{fig:BaseNonFedCP}
<<ScrutinyNonFedCP, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2, out.width='0.8\\linewidth'>>=
# Scrutiny Change Point
ScrutVars <- c("SumNonFed")
e.divGG(data = MainMonth, Vars = ScrutVars, TimeVar = "MonthYear", 
        sig.lvl = 0.05, R = 999, min.size = 30)
@
{\scriptsize{The red horizontal dashed line signifies the estimated change point at the 95\% significance level.\\
The data is aggregated on a monthly basis, i.e. observations represent monthly hearing counts.}}
\end{figure}
...
The results of the multivariate economic indicator change point analysis are in Figure \ref{fig:FullEconCP}. We estimate that there are seven states in the economic data from 2001 through 2012.\footnote{These dates were chosen to match the available Congressional hearing data.} A number of the states correspond to relatively minor changes in the economic indicators. For example, the second state from late 2003 through late 2005 corresponds to a relatively minor increase in inflation and a similarly gradual fall in unemployment. Other states, primarily the fourth and fifth correspond to fairly dramatic changes. These cover the Global Financial Crisis. 

When we compare the estimated scrutiny states to the economic states we see that the high scrutiny states between late 2007 and early 2010 closely corresponds to two economic states with the most severe changes--both the 2007/08 fall and the subsequent rise--in the observation period. There are two recessions in our sample. The first was in 2001 following the Dot Com crash and the second ran from late 2007 until mid-2009 and resulted from the Global Financial Crisis. As we can see in the Figure \ref{fig:FullEconCP} the first recession was relatively mild in terms of changes in inflation, unemployment and growth compared to the second. This indicates that Congress may only increase its scrutiny of the Fed and other economic policy bureaucrats when the economy is under extreme stress. Using McCubbins and Schwartz's \citeyearpar{Mccubbins1984} terminology, Congress increases its scrutiny when the fire alarm is ringing very loudly.
...
%SubMonth <- MainMonth[, c("MonthYear", "SumFedSenate", "FedLaughterSenate")]
