...
We gathered data on congressional hearings--in both the Senate and the House of Representatives--that included the presence of a representative of the Federal Reserve. We aimed to gather (a) all of the transcripts for the hearings where a member Federal Reserve representative gave testimony and (b) all of the transcripts for the hearings of both the US House of Representatives Committee on Financial Services (CFS)\footnote{From 1995 until 2000 it was known as the Committee on Banking and Financial Services.} and the Senate Committee on Banking, Housing, and Urban Affairs (BHUA). Both of these committees have the normal responsibility for oversight of the Federal Reserve.
...
\begin{figure}
    \caption{Congressional Scrutiny Change Point Analysis: US House of Representatives}
...
@
{\scriptsize{The red horizontal dashed line signifies the estimated change point at the 90\% significance level from an analysis including all of the variables.\\
The data is aggregated on a monthly basis with Laughter as the monthly mean.}}
\end{figure}


\begin{figure}
    \caption{Congressional Scrutiny Change Point Analysis: US House Financial Services Committee with No Fed Testimony}
    \label{fig:BaseNonFedCP}
<<ScrutinyNonFedCP, echo=FALSE, message=FALSE, warning=FALSE, out.width='0.8\\linewidth'>>=
# Scrutiny Change Point
ScrutVars <- c("SumNonFed", "NonFedLaughter")
...
e.divGG(data = MainMonth, Vars = EconVars, TimeVar = "MonthYear", 
        sig.lvl = 0.05, R = 999, min.size = 16)
@
