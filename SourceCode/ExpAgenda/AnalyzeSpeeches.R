...
TopicDoc <- DocTopics(Est1, StemLabel = 5)
...
# Create Chairman variable
SubCount$Chair <- 0
SubCount$Chair[SubCount$Names == "Alan Greenspan"] <- 1 
SubCount$Chair[SubCount$Names == "Ben S. Bernanke"] <- 1 

ggplot(SubCount, aes(Year, Freq, colour = Names, linetype = factor(Chair))) +
  geom_line() +
  facet_grid(Var1~.) +
  theme_bw()
